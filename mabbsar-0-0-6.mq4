...
input double ATRThresholdMedium = 0.0015; // Medium ATR threshold
input double ATRThresholdHigh = 0.0030;   // High ATR threshold
...
void OnTick()
{
    ...
    // Check ATR condition for opening orders
    bool atrConditionMet = false;

    if (currentATR > ATRThresholdMedium) {
        if (currentATR <= ATRThresholdHigh) {
    ...
    // Check for new trade signals with ATR condition
    if (currentFastMA > currentSlowMA && lastFastMA <= lastSlowMA && lastSignal != 1 && currentPrice > currentSAR && atrConditionMet)
    {
        // Buy signal with ATR condition met
    ...
    else if (currentFastMA < currentSlowMA && lastFastMA >= lastSlowMA && lastSignal != -1 && currentPrice < currentSAR && atrConditionMet)
    {
        // Sell signal with ATR condition met
        if (OrdersTotal() == 0) // Check if there are no open orders
        {
